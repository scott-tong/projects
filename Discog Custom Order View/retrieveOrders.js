...
function displayOrderList(){
	...
	$.getJSON("https://api.discogs.com/marketplace/orders?status=New Order&token=", function(data){
		var items = [];
		$.each(data["orders"], function(key,value){
			items.push("<div id='orderID'>Order Number: " + value['id'] + "</div>" );
			items.push("<a id='orderID' href=https://api.discogs.com/marketplace/orders/" + value['id'] + "?token=>" + "JSON Link</a>");
			items.push("<br/>")
